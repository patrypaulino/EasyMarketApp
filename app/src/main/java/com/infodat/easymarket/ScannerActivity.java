        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            if (result != 0) {
                ...
                for (Symbol sym : syms) {
                    ...
                    // Uri uriUrl = Uri.parse(scanResult); //Pasos para abrir la informacion (URL) del QR
                    // Intent launchBrowser = new Intent(Intent.ACTION_VIEW, uriUrl);
                    // startActivity(launchBrowser);//Abriendo el Browser para buscar la pagina.
            ...
            // Si el codigo de barra es valido, tomamos el valor escaneado
            // y lo retornamos al Activity padre (donde esta la lista de los prod escaneados)
            if (!scanResult.trim().equals("")) {
                releaseCamera();
                Intent returnIntent = new Intent();
                returnIntent.putExtra("scannedCode", scanResult);
